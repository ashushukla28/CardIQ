...
import { Pill, PrefTile, CardTagInput, StepDot, StepLine, BtnPrimary, CardResult } from "./components";
...
export default function App() {
  ...
  const analyze = async () => {
    ...
    const prompt = "Indian credit card advisor. User: " + empType + ", income " + income + ", spend Rs" + (monthly || "unspecified") + "/month, priorities: " + priorityLabels + ". Cards: " + top.map((c) => c.name).join(", ") + ". Return ONLY raw JSON: {\"synopses\":[\"why card1 fits\",\"why card2\",\"why card3\"],\"winner\":\"" + top[0].name + "\",\"winnerReason\":\"1 sentence\"}";
    try {
      const ctrl = new AbortController();
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"24px 16px 60px" }}>
        ...
        {step === "results" && results.length > 0 && (
          <div className="fade">
            ...
            {results.map((card, i) => (
              <CardResult key={card.id} card={card} i={i} priorities={priorities} synopses={synopses} monthly={monthly} openBreakdown={openBreakdown} setOpenBreakdown={setOpenBreakdown} />
            ))}
            <div style={{ background:"#fff", borderRadius:16, padding:18, border:"1px solid #e5e7eb", marginTop:4, fontSize:11, color:"#6b7280", lineHeight:1.8 }}>
              ...
              <p style={{ margin:"0 0 6px" }}><strong>Data accuracy:</strong> Card data is based on publicly available information as of mid-2025. Always verify current terms on the issuer website before applying.</p>
